...
--
-- Hôte : localhost:3306
-- Généré le : jeu. 07 sep. 2023 à 12:03
...
--
-- Doublure de structure pour la vue `v_GlobalCde`
...
--
-- Doublure de structure pour la vue `v_VentesI100`
-- (Voir ci-dessous la vue réelle)
--
...
--
-- Doublure de structure pour la vue `v_VentesI100Grobrigan`
-- (Voir ci-dessous la vue réelle)
--
...
--
-- Structure de la vue `v_GlobalCde`
--
DROP TABLE IF EXISTS `v_GlobalCde`;

CREATE ALGORITHM=UNDEFINED DEFINER=`admin`@`localhost` SQL SECURITY DEFINER VIEW `v_GlobalCde`  AS SELECT `ligcom`.`codart` AS `codart`, sum(`ligcom`.`qtecde`) AS `QteTot`, sum(`ligcom`.`qtecde`) * `ligcom`.`priuni` AS `PrixTot` FROM `ligcom` GROUP BY `ligcom`.`codart` ;

-- --------------------------------------------------------

--
-- Structure de la vue `v_VentesI100`
--
...
--
-- Structure de la vue `v_VentesI100Grobrigan`
--
DROP TABLE IF EXISTS `v_VentesI100Grobrigan`;

CREATE ALGORITHM=UNDEFINED DEFINER=`admin`@`localhost` SQL SECURITY DEFINER VIEW `v_VentesI100Grobrigan`  AS SELECT `vente`.`codart` AS `codart`, `vente`.`numfou` AS `numfou`, `vente`.`delliv` AS `delliv`, `vente`.`qte1` AS `qte1`, `vente`.`prix1` AS `prix1`, `vente`.`qte2` AS `qte2`, `vente`.`prix2` AS `prix2`, `vente`.`qte3` AS `qte3`, `vente`.`prix3` AS `prix3` FROM `vente` WHERE `vente`.`codart` = 'I100' AND `vente`.`numfou` = '00120' ;
